...
# Az van, hogy a data.table típusú objektum egy olyan data frame,
# ami az adatbuheráló műveleteket (pl. szűrés, aggregáció) párhuzamosítva tudja végezni
# Ezzel az alábbi paranccsal egyelőre azt engedélyezem, hogy a data.table objektumok az összes
# elérhető processzormagot használhatják (ez a 0-s kód). De később majd ezt limitálom egy magra,
# mivel ez összeakadna a párhuzamosan futtatott szimulációk magfoglalásával. Ez lenne az in inglis
# 'oversubscribing' probléma: két párhuzamosan futó algoritmus versengene a processzormagokért, és egyszerre akarnák
# ugyan azt a magot hazsnálni.
setDTthreads(0)
...
# A két külső ciklus rácsa: egy elem a rácsban egyetlen (Distribution1, Distribution2) párnak felel meg.
# Az utolsó oszlop (a seed) változik a leggyorsabban, ezért az eredmények sorainak sorrendje megegyezik az eredeti,
# egymásba ágyazott ciklusokat használó megoldás eredménysorrendjével.

pair_grid <- CJ(
  Distribution1 = dist_names,
  Distribution2 = dist_names,
  sorted = FALSE
)

# Worker függvény: a két belső ciklust helyettesíti
# (a name1 és name2 értéke a pair_grid létrehozása után már rögzített,
#  így a függvény egyetlen eloszláspárhoz tartozó seed ciklust hajtja végre).

simulate_pair <- function(i, pair_grid, n, seeds) {
  
  name1 <- pair_grid$Distribution1[i]
  name2 <- pair_grid$Distribution2[i]
...
#-----------------------------------------------------------------------------
# Párhuzamosított klaszter setup
#-----------------------------------------------------------------------------

n_cores <- max(1L, detectCores() - 1L) # maximálisan elérhető magok száma -1 --> 1 magot hagyok a Youtube-nak / Spotify-nak

cl <- makeCluster(n_cores)

# Ha a futás megszakad, a párhuzamosított klasztert manuálisan le kell állítani
# a stopCluster(cl) paranccsal, különben az R-folyamatok a háttérben tovább futnak.

# Mindegyik processzormag kapja meg a használt package-ket
invisible(clusterEvalQ(cl, {
  ...
  library(energy)
  library(data.table)
  # itt írom felül a data.table párhuzamosítását: csak 1 szálon, azaz sorosan futhat, hogy ne legyen oversubscribe
  # a szimulációs párhuzamosítással
  setDTthreads(1L)
  NULL
}))

# Mindegyik processzormag kapja meg a használt saját custom függvényeket is
clusterExport(
  cl,
  varlist = c(
    "mixture_normal",
...
# Minden 'n' mintaméretre párhuzamosítva futtatom a 'simulate_pair'-t
# Tehát az eloszláspárok szimuláció futnak párhuzamosan --> Ez szerintem a leghatékonyabb
...
# Minden eredményt egy data frame-be rakunk el
